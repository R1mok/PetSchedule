...
class UserMapperTest {
    ...
    void entityToDTO() {
        val users = List.of(
                User.builder().id(0).login("Vantuz").name("Иван").gender(Gender.MALE).passwordHash(42878).build(),
                User.builder().id(1).login("Jawa").about("Поделил на ноль").name("Джава").gender(Gender.MALE).passwordHash(42).build()
        );
